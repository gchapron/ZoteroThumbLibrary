...
Built for **Zotero 9.0.x and 10.0.x**. Version 0.1.6 was tested inside **Zotero 10.0.4 on macOS**, using an isolated profile and synthetic files. Earlier releases were tested inside Zotero 9.0.6; the Zotero 9 collection interface remains supported by a compatibility adapter and portable tests.
...
The current release includes 79 passing portable tests. Nine added compatibility tests cover plural collection contexts, idle refresh detection, collection changes during deletion, native drag cancellation, and tag permissions. Actual Zotero checks exercise card drag events through the native tag panel, multiple selection, Command-drop removal, numbered colored-tag toggling, range-selection anchoring, and the read-only shortcut guard.

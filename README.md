...
Built for **Zotero 9.0.x and 10.0.x**. Version 0.1.10 was tested inside **Zotero 10.0.4 on macOS**, using an isolated profile and synthetic files. Earlier releases were tested inside Zotero 9.0.6; the Zotero 9 collection interface remains supported by a compatibility adapter and portable tests.
...
- Drag files from Finder into empty space in icon view to import them into the current library or selected collections. Drop onto a parent-item card to attach the files to that item. As in list view, a normal drop copies files into Zotero; **⌘-drop** moves them and **⌘⌥-drop** links to the originals. Zotero's usual permissions, file renaming, and automatic metadata retrieval apply.
...
Tests cover large-library layout bounds, selection, attachment grouping, window reopening, scroll coalescing, zoom anchoring, reuse of 2,000 image elements, decoded-image lock limits and release, reduced motion, native item dragging, incoming file drops and native import delegation, colored-tag shortcuts, Delete-key handling and selection continuity, QuickLook shortcut forwarding, EPUB cover extraction, shared attachment jobs, source-file invalidation, the 2,000-file disk limit, cache recovery, independent cache reads during a stalled PDF render, renderer concurrency and per-worker failure isolation, range reads and cancellation, viewport priority, rapid scrolling, and hidden-renderer scheduling.

The current build includes 127 passing portable tests. Compatibility tests cover plural collection contexts, idle refresh detection, collection changes during deletion and file imports, native drag cancellation, and tag permissions. File-drop tests cover native row targeting, import modifiers, rejected and internal drags, changed contexts, error handling, and listener cleanup. Earlier actual Zotero checks exercise card drag events through the native tag panel, multiple selection, Command-drop removal, numbered colored-tag toggling, range-selection anchoring, and the read-only shortcut guard.

`tests/file-drop-runtime-result.json` records the packaged 0.1.10 checks inside Zotero 10.0.4: imports into an empty collection, matching stored bytes and preserved sources, new cards appearing, attachments dropped onto parent cards, Command-move, Command-Option-link, and rejection of unsupported targets and read-only libraries. `tests/runtime-file-drop.js` dispatches synthetic native-file drag events in a guarded disposable profile; it does not automate a Finder gesture. Multiple-file imports were not runtime-tested because Gecko's constructed transfer rejected a second native file. The read-only warning display was stubbed while the native permission check remained active.
